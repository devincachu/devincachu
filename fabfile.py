...
def start_gunicorn():
    with cd(env.app_root):
        run('%(virtualenv)s/bin/gunicorn_django --pid=gunicorn.pid --daemon --workers=1 --access-logfile=devincachu_access.log --error-logfile=devincachu_error.log --bind=127.0.0.1:8888' % env)
...
def restart_nginx():
    run("su -m root -c '/usr/local/sbin/nginx -s stop && /usr/local/sbin/nginx'")
